...
export class SeederService implements OnModuleInit {
  ...
  async seed() {
    ...
    // 1. 创建示例知识库
    const sampleKb = await this.kbRepo.save(
      this.kbRepo.create({
        name: 'LOOM 产品指南',
    ...
    // 2. 创建示例工作流（关联到知识库）
    const sampleWorkflow = await this.workflowRepo.save(
    ...
    // 3. 创建示例文档
    const sampleDoc = await this.docRepo.save(
      this.docRepo.create({
        name: '产品介绍.txt',
    ...
    const sampleChunks = [
      {
        content:
          'LOOM 是一个轻量级的智能体平台，支持可视化工作流编排。用户可以通过拖拽节点来创建复杂的工作流程。',
        order: 0,
      },
      {
        content:
          '平台提供知识库管理功能，支持上传 TXT 和 Markdown 格式的文档。文档会自动进行分片处理，便于精确检索。',
        order: 1,
      },
      {
        content:
          'RAG（检索增强生成）技术是平台的核心能力。系统会将文档转换为向量存储，当用户提问时，自动检索相关片段并注入到 AI 上下文中。',
        order: 2,
      },
      {
        content:
          '工作流节点类型包括：触发器（Trigger）用于启动工作流，AI 节点用于调用大模型，知识检索节点用于 RAG，分支节点用于条件判断。',
        order: 3,
      },
      {
        content:
          '智能问答功能支持多轮对话，系统会记住对话历史。用户可以在聊天窗口中提出问题，AI 会基于知识库中的内容进行回答。',
        order: 4,
      },
    ];
    ...
    // 5. 创建第二个知识库 - 常见问题
    const faqKb = await this.kbRepo.save(
      this.kbRepo.create({
        name: '常见问题 FAQ',
    ...
    const faqDoc = await this.docRepo.save(
      this.docRepo.create({
        name: 'FAQ.md',
        knowledgeBase: faqKb,
      }),
    ...
    const faqChunks = [
      {
        content:
          'Q: 如何创建工作流？\nA: 点击左侧的"工作流编排" tab，然后从节点面板拖拽所需的节点到画布上，连接节点后即可。',
        order: 0,
      },
      {
        content:
          'Q: 如何上传知识库文档？\nA: 进入"知识库管理"，点击"创建知识库"按钮创建一个新的知识库，然后点击知识库卡片上的"上传文档"按钮上传 TXT 或 Markdown 文件。',
        order: 1,
      },
      {
        content:
          'Q: RAG 是什么意思？\nA: RAG 是 Retrieval-Augmented Generation（检索增强生成）的缩写。它是一种结合检索和生成的技术，可以从知识库中检索相关文档片段，再让 AI 基于这些片段生成回答。',
        order: 2,
      },
      {
        content:
          'Q: 工作流执行失败怎么办？\nA: 检查节点的配置是否正确，查看底部的运行日志了解具体错误信息。常见问题包括：API Key 未配置，知识库 ID 不存在等。',
        order: 3,
      },
    ];
    ...
    this.logger.log(
      `Created FAQ knowledge base with ${faqChunks.length} chunks`,
    );
  ...
  private computeSimpleEmbedding(text: string): number[] {
    // 与 KnowledgeService.computeEmbedding 保持一致
    const DIM = 384;
