...
export class ExecutorService {
  ...
  constructor(
    ...
    // 打印已注册的策略（调试用）
    this.logger.log(
      `Registered condition strategies: ${this.conditionContext.getRegisteredStrategies().join(', ')}`,
    );
  }
  ...
  async runWorkflow(
    ...
    this.logger.log(
      `Starting workflow execution: ${workflow.name} (${workflow.id})`,
    );
    ...
    // 更新工作流日志状态
    const updatedLog = await this.workflowLogRepo.findOneBy({
      id: workflowLog.id,
    });
    if (updatedLog) {
      updatedLog.status = hasError
        ? WorkflowLogStatus.FAILED
        : WorkflowLogStatus.COMPLETED;
      updatedLog.outputData = JSON.stringify({
    ...
    switch (nodeType) {
      ...
      case NodeType.END: {
        ...
        for (const [key, value] of Object.entries(input)) {
          if (key === '_context' || key === 'input' || key === 'START_INPUT')
            continue;
        ...
        this.logger.warn(
          `[OUTPUT node] No AI output found in input keys: ${Object.keys(input).join(', ')}`,
        );
        return { text: '' };
      }
      default:
        // 未知类型节点默认透传
        this.logger.warn(
          `Unknown node type: ${nodeType}, passing through input`,
        );
        return input;
    }
  }
  ...
  private async handleAIRequest(
    ...
    this.logger.debug(
      `[AI Node ${node.id}] Input keys: ${Object.keys(input).join(', ')}`,
    );

    // 简单的变量插值: {{nodeId}} 或 {{nodeId.output}}
    if (input._context) {
      this.logger.debug(
        `[AI Node ${node.id}] Context keys: ${Object.keys(input._context).join(', ')}`,
      );
    ...
    this.logger.log(
      `[AI Node ${node.id}] Final prompt: ${prompt.substring(0, 200)}...`,
    );

    try {
      // 流式调用
      const model =
        config.model || process.env.LLM_MODEL || 'deepseek-ai/DeepSeek-V3';
  ...
  private async handleKnowledgeRetrieval(
    node: NodeData,
    config: any,
    input: any,
  ): Promise<any> {
    ...
    this.logger.log(
      `[Knowledge Retrieval Node ${node.id}] Final query: ${query.substring(0, 200)}...`,
    );
  ...
  private async handleCondition(
    ...
    // 使用策略模式执行条件求值
    const { result, strategy } = this.conditionContext.evaluate(
      input,
      expression,
    );
    ...
    this.logger.log(
      `Condition node ${node.id} result: ${conditionResult} (using ${strategy} strategy)`,
    );
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        ...
        if (resp.status >= 400 && resp.status < 500) {
          this.logger.warn(
            `[HTTP Node ${node.id}] 客户端错误 ${resp.status}，停止重试`,
          );
          break;
        }
